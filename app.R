...
suppressPackageStartupMessages({
  ...
  library(readxl)
  library(ggplot2)   # 目前未用到图，但保留以兼容后续扩展
  library(plotly)    # 同上
  library(DT)        # 代码里使用 renderTable，但DT保留以兼容后续扩展
  # library(openxlsx) # 未使用 → 移除不必要依赖（不影响逻辑）
  # library(writexl)  # 未使用 → 移除不必要依赖（不影响逻辑）
})

# ======= 全局常量/样式集中（不改变规则，仅抽出，便于复用维护）=======
STYLE_RISK <- list(
...
# LDL 目标卡片也补齐 icon，修复此前 icon(NULL) 的潜在风险
STYLE_LDL <- list(
  "低危"         = list(color = "#69b3a2", icon = "smile"),
  "中危"         = list(color = "#009ACD", icon = "meh"),
  "高危"         = list(color = "goldenrod", icon = "frown"),
...
ui <- dashboardPage(
  skin = "blue",
  dashboardHeader(title = HTML("AtheroTarget")),
  dashboardSidebar(
    sidebarMenu(
      id = "tabs",
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(
        tabName = "shuru",
        fluidRow(
          column(2, numericInput("age", "年龄（岁）", value = 50, min = 1, max = 120)),
          column(2, selectInput("gender", "性别", choices = c("男", "女"), selected = "男")),
          column(2, selectInput("hypertension", "高血压", choices = c("是", "否", "不详"), selected = "否")),
          column(2, selectInput("diabetes", "糖尿病", choices = c("是", "否", "不详"), selected = "否")),
          column(2, selectInput("type1diabetes", "1型糖尿病≥20年", choices = c("是", "否", "不详"), selected = "否")),
          column(2, selectInput("smoking", "吸烟", choices = c("是", "否", "不详"), selected = "否"))
        ),
        
        # 仅在“高血压=是”时显示SBP/DBP（布局保持不变）
        conditionalPanel(
          condition = "input.hypertension == '是'",
          fluidRow(
            column(2, numericInput("sbp", "收缩压（mmHg）", value = 140, min = 100, max = 300)),
            column(2, numericInput("dbp", "舒张压（mmHg）", value = 90,  min = 30,  max = 200))
          )
        ),
        ...
        fluidRow(
          box(
            ...
            fluidRow(
              ...
              column(
                width = 4,
                selectInput("dialysis", "透析状态", choices = c("是", "否", "不详"), selected = "否")
              )
            ),
              ...
              fluidRow(
                box(
                  ...
                  fluidRow(
                    column(
                      width = 4,
                      numericInput("scr_umol", "血清肌酐 (μmol/L)", value = 80, min = 10, max = 2000, step = 1)
                    ),
                    column(
                      width = 2,
                      div(
                    ...
                    column(
                      width = 4,
                      numericInput("egfr", "eGFR (mL/min/1.73m²)", value = NA, min = 0, max = 200, step = 0.1)
                    )
                  )
                )
              )
            ...
            fluidRow(
              column(4, selectInput("ascvd", "ASCVD", choices = c("是", "否", "不详"), selected = "不详")),
              column(8,
            ...
            conditionalPanel(
              ...
              box(
                ...
                fluidRow(
                  column(3, selectInput("ldl_recurrent", "LDL ≤1.8 mmol/L 时再发严重ASCVD事件", choices = c("是","否","不详"), selected = "不详")),
                  column(3, selectInput("early_cad", "早发冠心病（男<55, 女<65）", choices = c("是","否","不详"), selected = "不详")),
                  column(3, selectInput("fh_high_ldl", "家族性高胆固醇血症或基线 LDL≥4.9", choices = c("是","否","不详"), selected = "不详")),
                  column(3, selectInput("prior_revascularization", "既往行冠状动脉支架/搭桥", choices = c("是","否","不详"), selected = "不详"))
                )
              )
            )
          )
        ...
        fluidRow(
          column(
            ...
            actionButton(
              ...
              style = paste("width:100%; font-size:22px; color:#FFFFFF; display:flex; justify-content:center; align-items:center;")
            )
          )
        ),
        fluidRow(
          column(
            ...
            actionButton(
              ...
              style = paste("width:100%; font-size:22px; color:#FFFFFF; display:flex; justify-content:center; align-items:center; margin-top:10px;")
            )
          )
        )
      ),
        ...
        fluidRow(
          column(
            ...
            actionButton(
              ...
              class = "btn-success btn-lg",
              style = paste("width:100%; font-size:22px; color:#FFFFFF; display:flex; justify-content:center; align-items:center; margin-top:20px;")
            )
          )
        )
      ),
      ...
      tabItem(
        ...
        fluidRow(
          box(
            width = 12, solidHeader = TRUE, status = "info", class = "version-box",
            tags$div(" ", class = "version-header"),
            tags$div(
              class = "box-body",
              tags$div(
                class = "version-meta",
                tags$p(HTML("<strong>版本：</strong>AtheroTarget-低密度脂蛋白管理与心血管风险控制精准平台(V1.2)"),
                       style = "font-size:22px; font-weight:bold;"),
                tags$p(HTML("<strong>作者：</strong>刀客特lin"),
                       style = "font-size:22px; font-weight:bold;")
              ),
              tags$div(
...
server <- function(input, output, session) {
  ...
  # 小工具：批量更新 Select/Numeric（不改变行为，只减少重复）
  batch_update_select <- function(vals) {
    lapply(names(vals), function(id) updateSelectInput(session, id, selected = vals[[id]]))
  }
  batch_update_numeric <- function(vals) {
    lapply(names(vals), function(id) updateNumericInput(session, id, value = vals[[id]]))
  }
  ...
  observe({
    ...
    # 2) 高血压相关校验：只有在 “高血压=是” 才校验 SBP/DBP
    warn_sbp <- FALSE
    warn_dbp_range <- FALSE
    warn_dbp_order <- FALSE
    if (!is.null(input$hypertension) && input$hypertension == "是") {
    ...
    # 3) 1型糖尿病校验：若1型糖尿病=是，则糖尿病必须=是
    warn_t1d <- !is.null(input$type1diabetes) && input$type1diabetes == "是" && input$diabetes != "是"
    feedbackWarning("type1diabetes", warn_t1d, "若选择“是”，则“糖尿病”必须为“是”")
    
    # 4) 非ASCVD 时的其他校验（保持原规则）
    warn_other <- FALSE
    ...
    # 5) 有任一警告则禁用计算按钮
    any_warn <- warn_age || warn_sbp || warn_dbp_range || warn_dbp_order || warn_t1d || warn_other
    if (any_warn) shinyjs::disable("calculate") else shinyjs::enable("calculate")
  })
  # ======= 恢复默认值=======
  observeEvent(input$reset, {
    batch_update_numeric(list(
      age = 50, sbp = 140, dbp = 90,
      tc = 5.0, ldl = 3.0, hdl = 1.2,
      height = 170, weight = 65,
      scr_umol = 80, egfr = NA
    ))
    batch_update_select(list(
      gender = "男", hypertension = "否", diabetes = "否", smoking = "否",
      type1diabetes = "否", ascvd = "不详", ckd = "否", dialysis = "否",
      recent_acs = "不详", prior_mi = "不详", stroke_history = "不详", pvd = "不详",
      ldl_recurrent = "不详", early_cad = "不详", fh_high_ldl = "不详", prior_revascularization = "不详"
    ))
  })
  ...
  observeEvent(input$calc_egfr, {
    ...
    egfr_val <- 186 * (scr_mgdl ^ -1.154) * (input$age ^ -0.203) * ifelse(input$gender == "女", 0.742, 1.000)
    updateNumericInput(session, "egfr", value = round(egfr_val, 1))
    updateSelectInput(session, "ckd", selected = if (egfr_val < 60) "是" else "否")
  })
  ...
  observeEvent(input$calculate, {
    ...
    if (!is.null(input$ascvd) && input$ascvd == "是") {
      ...
      cond1 <- (events_yes >= 2) || (events_yes >= 1 && factors_yes >= 2)
      cond2 <- (events_no == 4) || (events_yes == 1 && events_no == 3 && factors_no >= 7)
      
      if (cond1)      initial_cat <- "超高危"
      else if (cond2) initial_cat <- "确定的极高危"
      else            initial_cat <- "可能的极高危"
      ...
      if ((input$tc >= 7.2) ||
          ...
          (input$ckd == "是") ||
          (input$diabetes == "是" && input$age < 40 &&
           input$hypertension == "是" && input$smoking == "是" &&
           !is.na(input$hdl) && input$hdl < 1.0)) {
        initial_cat <- "高危"
      } else {
        # 高血压 + 危险因素 + 胆固醇分档
        ...
        TC  <- input$tc
        LDL <- input$ldl
        tc_band <- if      (TC >= 3.1 && TC < 4.1) 1 else if (TC >= 4.1 && TC < 5.2) 2 else if (TC >= 5.2 && TC < 7.2) 3 else NA
        ldl_band <- if     (LDL >= 1.8 && LDL < 2.6) 1 else if (LDL >= 2.6 && LDL < 3.4) 2 else if (LDL >= 3.4 && LDL < 4.9) 3 else NA
        cholesterolBand <- if (all(is.na(tc_band), is.na(ldl_band))) NA else max(tc_band, ldl_band, na.rm = TRUE)
        
        if (is.na(hasHighBP) || is.na(riskFactors) || is.na(cholesterolBand)) {
          initial_cat <- "信息不完整"
        } else if (!hasHighBP) {
          if (riskFactors <= 1) {
            initial_cat <- "低危"
      ...
      if (initial_cat == "中危" && input$age < 55) {
        ...
        if (!is.na(input$tc) && !is.na(input$hdl) && (input$tc - input$hdl) >= 5.2) extra_factors <- extra_factors + 1
    ...
    # 危险分层卡片（使用集中样式；仅展示，不改变内容）
    output$riskStratText <- renderUI({
      req(riskCategory())
      cat  <- riskCategory()
      cfg  <- STYLE_RISK[[cat]]; if (is.null(cfg)) cfg <- list(color = "light-blue", icon = "question")
      noteLine <- nzchar(note)
      
      tags$div(
        style = paste0("background:", cfg$color, "; color:white; padding:20px; border-radius:6px; text-align:center;"),
        tags$h3(icon(cfg$icon), " ", cat, style = "margin:0; font-weight:bold; font-size:2em;"),
        if (noteLine) {
          tags$p(icon("exclamation-triangle"), "余生危险风险为高危",
                 style = "color: red; font-weight:bold; margin-top:10px;")
        }
      )
    })
    
    # LDL 目标卡片（补齐 icon 的同时保持原逻辑）
    output$ldlTargetText <- renderUI({
      req(riskCategory())
      cat <- riskCategory()
      cfg <- STYLE_LDL[[cat]]; if (is.null(cfg)) cfg <- list(color = "light-blue", icon = "question")
      ...
      tags$div(
        style = paste("background:", cfg$color, "; color:white; padding:20px; border-radius:6px; text-align:center;"),
        tags$h3(icon(cfg$icon), " ", target_text, style = "margin:0; font-weight:bold;")
      )
    })
    ...
    output$warningsUI <- renderUI({
      ...
      cond_unclear <- ((!is.null(input$ascvd) && input$ascvd == "是" && initial_cat == "可能的极高危") ||
                         (is.null(input$ascvd) || input$ascvd != "是"))
      hist_fields <- c(
        input$diabetes, input$smoking, input$ckd, input$hypertension, input$ascvd, input$type1diabetes,
        input$recent_acs, input$prior_mi, input$stroke_history, input$pvd,
        input$ldl_recurrent, input$early_cad, input$fh_high_ldl, input$prior_revascularization
      )
      if (cond_unclear && any(hist_fields == "不详")) {
        msgs <- c(msgs, "检测到患者的部分病史为“不详”，目前评估仅为初步评估，若需更加精确的判断，需明确“不详”的病史，或根据临床表现进一步完善相关检查。")
      }
      if (!is.null(input$ascvd) && input$ascvd != "是" && input$dialysis == "是") {
        msgs <- c(msgs, "检测到患者为透析状态的非ASCVD患者，不建议使用他汀类药物。")
      }
      if (!is.null(input$ascvd) && input$ascvd != "是" && input$diabetes == "是" && input$age < 40) {
        msgs <- c(msgs, "检测到患者为40岁以下的糖尿病患者，只要出现明确靶器官损害（如蛋白尿、肾功能损害、神经病变或视网膜病变等），即为高危，LDL-c需控制在1.8 mmol/L以下。")
      }
      ...
      tags$div(
        lapply(msgs, function(m) {
          tags$div(
            style = paste(
              "background-color:#FFFFFF; color:#800020; font-weight:bold; font-size:18px;",
              "border-radius:6px; margin-bottom:10px; display:flex; align-items:center; padding:.75rem 1.25rem;"
            ),
    ...
    output$treatmentAdvice <- renderUI({
      ...
      if (cat %in% c("低危", "中危") && !noteLine) {
        ...
        if (cat == "超高危") {
          advice <- c(advice, "6. 若基线 LDL-C ≥4.9 mmol/L，或服用他汀后 LDL-C ≥2.6 mmol/L 且预计中等强度他汀+吸收抑制剂不能达标，可直接启动他汀+PCSK9抑制剂。")
        }
      }
      ...
      tags$div(
        ...
        lapply(advice, function(txt) {
          tags$p(icon("check-circle", class = "fa-lg", style = "color:#28a745;"),
                 span(txt, style = "font-size:18px; font-weight:bold; margin-left:8px;"))
        })
      )
    })
  })
